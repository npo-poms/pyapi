...
class Media(NpoApi):
    def get(self, mid, sub="", sort=None, accept=None, properties=None, limit=None, offset=None, profile=None, stream=False) -> Union[None, http.client.HTTPResponse, str]:
    ...
    def changes_raw(self, profile=None, order="ASC", stream=False, limit=10, since=None, force_oldstyle=False, properties=None, check_profile=None, deletes="ID_ONLY", tail=None, reason_filter=None) -> Union[None, http.client.HTTPResponse, str]:
        if isinstance(properties, list):
            properties = ",".join(properties)
        sinceLong = None
        sinceDate = None
        ...
        params = { "profile": profile, "order": order, "max": limit,
                   ...
                   "checkProfile": check_profile, "deletes": deletes, "tail": tail, "reasonFilter": reason_filter }
        if stream:
