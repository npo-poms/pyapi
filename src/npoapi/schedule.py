...
class Schedule(NpoApi):
    def get_schedule_response(
        self, guideDay=None, channel=None, sort="asc", offset=0, limit=240, properties=None, accept=None
    ) -> Optional[str]:
        params = {"guideDay": guideDay, "sort": sort, "max": limit, "offset": offset, "properties": properties}
        if channel:
    ...
    def get_object(self, guideDay=None, channel=None, sort="asc", offset=0, limit=240, properties=None):
        response = self.get_schedule_response(
            guideDay, channel, sort, offset, limit, properties, accept="application/xml"
        )
