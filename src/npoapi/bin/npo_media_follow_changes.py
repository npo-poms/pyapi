...
class FollowChanges:
    def __init__(self):
        self.client = Media().command_line_client(
            "Get changes feed from the NPO Frontend API", exclude_arguments={"accept"}
        )
        ...
        self.client.add_argument(
            ...
            type=str,
            default=None,
            help="From what time/mid to stream, or a file containing it. It defaults to a file called since.<env>. Empty string: now, no store.",
        )
        ...
        self.client.add_argument("-p", "--properties", type=str, default=None, help="properties filtering")
        self.client.add_argument(
            "--raw",
            action="store_true",
            help="No attempts to stream and handle big results. Everything should fit in memory. Simpler, but less efficient.",
        )
        self.client.add_argument(
            "--reasonFilter",
            type=str,
            default="",
            help="reason filtering argument. Defaults to empty string (only reporting)",
        )
        self.client.add_argument(
            "--change_to_string",
            type=str,
            default="CONCISE",
            help="dict to string for change. E.g. 'change.get('id', '') + title(change). Or 'CONCISE' for a default concise string.",
        )
    ...
    def timestamp_to_string(timestamp):
        return (
            datetime.fromtimestamp(timestamp / 1000).isoformat(timespec="milliseconds") if not timestamp is None else ""
        )
    ...
    def one_call(self):
        self.logger.debug(
            "%s One_calling for %s/%s/%s"
            % (
                datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
                self.since["timestamp"],
                datetime.fromtimestamp(self.since["timestamp"] / 1000).strftime("%Y-%m-%dT%H:%M:%SZ"),
                self.since["mid"],
            )
        )
        response = self.client.changes_raw(
        ...
        for lazy_change in changes:
            ...
            self.logger.debug(
                "Setting since to %s (%s), (%s ms before now)"
                % (new_since, str(is_tail), int(datetime.now().timestamp() * 1000 - new_since))
            )
    ...
    def follow_changes(self):
        ...
        while True:
            try:
                self.logger.debug(
                    "since: %s,%s (%s)"
                    % (self.since, self.since["timestamp"], self.timestamp_to_string(self.since["timestamp"]))
                )
                if self.args.raw:
                    self.one_call_raw()
                else:
