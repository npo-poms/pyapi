...
from npoapi.utils import MID_HELP, resolve_mid


def mediabackend_get():
    client = MediaBackend().command_line_client(
        description="Get an media object from the NPO Backend API", exclude_arguments="errors"
    )
    ...
    client.add_argument(
        "-p", "--process", type=str, help="""python code to postprocess. E.g. "update.duration='PT5M'""" ""
    )
    ...
    client.add_argument("-D", "--delete", action="store_true", help="""The mid is deleted""")
    ...
    def get():
        ...
        if sub == "subtitles":
            ...
            return client.members_or_episodes(
                mid, sub, full=args.full, follow_merges=not args.nofollowMerges, deletes=args.deletes, raw=args.raw
            )
        else:
    ...
    if type(result) == list:
        strings = map(lambda o: o.toprettyxml(), result)
        for s in strings:
            print(s)
