...
def mediabackend():
    ...
    client.add_argument("-S", "--search", action="store_true", help="""Force that the xml is interpreted as a search""")
    client.add_argument("-W", "--writable", action="store_true", help="""Only writable items are searched""")
    client.add_argument("-D", "--delete", action="store_true", help="""The object with given mid will be deleted.""")

    client.add_argument(
        "--raw",
        action="store_true",
        help="""The XML will not be parsed first. Implies that the argument is XML,  not a MID""",
    )
    ...
    client.add_argument(
        "--steal_crids", type=str, default="IF_DELETED", help="""""", choices=["YES", "NO", "IF_DELETED"]
    )
                ...
                if not (type(update) == str) and xml.childNodes[0].namespaceURI == "urn:vpro:media:search:2012":
                    ...
                    print(
                        client.find(
                            ...
                            client_validate=args.client_validate,
                        )
                    )
                else:
                    if args.delete:
