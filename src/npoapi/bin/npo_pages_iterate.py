...
def pages_iterate():
    ...
    client.add_argument(
        "profile", type=str, nargs="?", help="The profile to search within. Can be empty string to not use a profile"
    )
    client.add_argument(
        "form",
        type=str,
    ...
    client.add_argument(
        ...
        type=int,
        default="100",
        help="On default the size is maximized to 100, but unlike with other API calls you can set this max value arbitrary large.",
    )
    client.add_argument(
        "--progress", action="store_true", help="If set to true, some progress indication will be written to stderr"
    )
    ...
    client.add_argument(
        ...
        type=int,
        default=None,
        help="If this is used, the 'output' parameter must contain a '%d', and every file will contain an array with this many media objects. Defaults to 1000 if output contains a '%d'",
    )
