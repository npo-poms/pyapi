...
def media_frontendsearch():
    ...
    client.add_argument(
        ...
        type=str,
        nargs=1,
        help="The search form. This may be a json string, or the name of a file containing it. If -, read from stdin",
    )
    client.add_argument("mid", type=str, nargs="?", default=None, help="Mid for use with 'sub'")
    client.add_argument(
        "sub",
        type=str,
        nargs="?",
    ...
    if not looks_like_form(form):
        form = '{"searches": {"text": {"value": %s, "fuzziness": "%s" }}}' % (
            json.dumps(form),
            "AUTO" if args.fuzzy else "",
        )
