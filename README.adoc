...
..bash_profile
-----
alias npoapi="docker run -it  -v ~/conf:/root/conf -v  $(pwd):/workdir mihxil/npo-pyapi"
-----
...
POMS API's often support both XML and JSON. Sometimes only XML (backend api's), and sometimes only JSON (changes feeds).

A generic binding to and from JSON would probably require manual tweaking. The poms java domain objects are annotated with JAXB and Jackson annotations  to arrange json bindings. The information and customizations contained in the jackson annotations are not available in the XML schema's, or available in an other way (besided the java code itself).
